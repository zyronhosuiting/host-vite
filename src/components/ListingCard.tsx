  ...
  return (
    ...
    >
      ...
      <div className="relative card-img flex-shrink-0 w-[110px] sm:w-auto">
        ...
        >
          ...
          {l.badge === '新上架' && isNewListing(l.listedDate) && (
            <span className="absolute top-2 left-2 sm:top-3 sm:left-3 bg-white text-slate text-xs font-bold px-2 sm:px-3 py-0.5 sm:py-1 rounded-pill z-10">
              {l.badge}
            </span>
          )}
        </PhotoCarousel>
      ...
      <div className="flex flex-col gap-1 flex-1 min-w-0 p-3 sm:p-4">
        ...
        <p className="text-md text-t2 leading-snug line-clamp-2">{cleanLoc(l.location)}</p>
        <p className="text-sm text-t3 hidden sm:block">{l.subtitle}</p>
